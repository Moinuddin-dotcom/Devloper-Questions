...
export default function LikeSection({ card }: LikeSectionProps) {
    ...
    const fetchQusData = async () => {
        try {
            const { data } = await axios(`${process.env.NEXTAUTH_URL}/api/single-blog/${postId}`);
    ...
    const updateLike = async () => {
        try {
            ...
            const response = await axios.patch(`${process.env.NEXTAUTH_URL}/api/single-blog/${postId}/upvote`, {
                user: userEmail,
            });
