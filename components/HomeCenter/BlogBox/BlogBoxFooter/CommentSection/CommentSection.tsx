...
export default function CommentSection({ card }: CommentSectionProps) {
    ...
    const fetchQusData = async () => {
        // setLoading(true);
        try {
            const { data } = await axios(`${process.env.NEXTAUTH_URL}/api/single-blog/${card?._id}`);
    ...
    const onSubmit = async (data: FormData) => {
        ...
        try {
            const response = await axios.patch(`${process.env.NEXTAUTH_URL}/api/single-blog/${card?._id}/comment`, commentUserData);
